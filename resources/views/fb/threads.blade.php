...
<h1>Facebook Inbox</h1>

<div class="top">
    <form method="GET" action="{{ route('fb.threads') }}">
        <input name="q" value="{{ $q }}" placeholder="Search PSID / text...">
...
@foreach($rows as $r)
    ...
    <div class="row">
        <div>
            ...
            <a href="{{ route('fb.threads.show', $r->thread_id) }}">{{ $r->thread_id }}</a>
        </div>
        <div class="muted">
            Last: {{ $r->last_sent_at }} |
            Total: {{ $r->msg_count }}
        ...
        @if($last)
            <div style="margin-top:8px;">
                <strong>{{ $last->direction === 'out' ? 'You' : 'User' }}:</strong>
