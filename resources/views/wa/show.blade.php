...
<head>
    <meta charset="utf-8">
    <title>WhatsApp Chat {{ $threadId }}</title>
    <style>
        ...
        .out .bubble{background:#e6fff1;border-color:#c7f5dc;}
...
<div class="wrap">
    ...
    <div class="top">
        <div>
            <a href="{{ route('wa.threads') }}">← Back</a>
            <h2 style="margin:8px 0;">WhatsApp: {{ $threadId }}</h2>
        </div>
    </div>
    ...
    <form method="POST"
          action="{{ route('wa.threads.send', $threadId) }}"
          style="margin-top:14px;">
        @csrf

        <label><strong>Reply WhatsApp</strong></label>
        <textarea name="text" placeholder="Type a WhatsApp message...">{{ old('text') }}</textarea>
